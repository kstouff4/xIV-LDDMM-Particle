...
def main():
    ...
    for t in range(len(Dlist)):
        ...
        if t == len(Dlist) - 1:
            if not torch.is_tensor(D):
                np.savez(
                    os.path.join(savedir, "testOutput.npz"),
                    D=D,
                    nu_D=nu_D,
                    nu_Dpi=nu_Dpi,
                )
    ...
    getLocalDensity(
        S, nu_S, sigmaVar0, os.path.join(savedir, "density_S.vtk"), coef=2.0
    )
    getLocalDensity(
        T, nu_T, sigmaVar0, os.path.join(savedir, "density_T.vtk"), coef=2.0
    )
    getLocalDensity(
        Dlist[-1],
        nu_Dlist[-1],
        sigmaVar0,
        os.path.join(savedir, "density_D.vtk"),
        coef=2.0,
    )
    ...
    jFile = gO.getJacobian(
        ...
        nu_S,
        nu_Dlist[-1],
        os.path.join(savedir, "testOutput_D10_jacobian.vtk"),
    )
    gO.splitZs(
        T,
        nu_T,
        Dlist[-1],
        nu_DPilist[-1],
        os.path.join(savedir, "testOutput_Dpi10"),
        units=15,
        jac=jFile,
    )
    ...
    getLocalDensity(
        x, nu_S, sigmaVar0, os.path.join(savedir, "density_D_ATau.vtk"), coef=2.0
    )
