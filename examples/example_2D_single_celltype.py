...
def main():
    ...
    savedir = os.path.join(
        "output", "MERFISH_Celltypes", f"2DdefaultParameters_{legacy}"
    )
    ...
    S, nu_S = torch.load(
        os.path.join(datadir, "source_2D_celltype.pt"), map_location=map_location
    )
    T, nu_T = torch.load(
        os.path.join(datadir, "target_2D_celltype.pt"), map_location=map_location
    )
    ...
    (
        ...
        dimEff,
        single,
    ) = torch.load(os.path.join(datadir, "parameters_2D_celltype.pt"))
    N = S.shape[0]
    ...
    getLocalDensity(
        Dlist[-1], nu_Dlist[-1], sigmaVar0, savedir + "density_D.vtk", coef=2.0
    )
