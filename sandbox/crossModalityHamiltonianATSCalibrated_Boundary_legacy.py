...
def printCurrentVariables(
    ...
    if supportWeightF is not None:
        ...
        nu_D = (
            wS.cpu().numpy()
            * np.squeeze(muD[0:numS])[..., None]
            * zeta_S.detach().cpu().numpy()
        )
        zeta_D = nu_D / np.sum(nu_D, axis=-1)[..., None]
        nu_Dpi = nu_D @ pi_ST
        Ds = init.resizeData(D, s, m)
        ...
        gO.writeVTK(
            ...
            imageValsS,
            imageNamesS,
            savedir + "testOutputiter" + str(itCurr) + "_D10Support.vtk",
            polyData=None,
        )
        gO.writeVTK(
            Ds,
            imageValsSpi,
            imageNamesSpi,
            savedir + "testOutputiter" + str(itCurr) + "_Dpi10Support.vtk",
            polyData=None,
        )
...
def callOptimize(
    ...
    def printCost(currIt):
        ...
        ax.plot(
            np.arange(len(lossListH)),
            np.asarray(lossListL),
            label="Lambda Cost, Final = {0:.6f}".format(lossListL[-1]),
        )
        ax.set_title("Loss")
