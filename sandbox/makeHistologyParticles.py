...
def main():
    ...
    imgfils = ['/cis/home/kstouff4/Documents/datasets/exvivohuman_11T/more_blocks/Brain3/histology/Restain_2023/AD_Hip1/BR_3_Bl_1_L1_3.TIF',
    ...
    imgfils = ['/cis/home/kstouff4/Documents/datasets/exvivohuman_11T/more_blocks/Brain3/histology/Restain_2023/AD_Hip3/BR1_BL3_L_1_3.TIF',
    ...
    for i in range(len(imgfils)):
        ...
        S,nu_S = gI.makeBinsFromDistance(imgfile,res,2,[155,110,100],ds=16,z=(i+1.0),threshold=10,bins=20,weights=[1,1,1])
    #S,nu_S = gI.makeFromSingleChannelImage(imgfile,res[0],bg=[0],ordering=[1,2,3],ds=1)
    
        S = S.detach().cpu().numpy()
        nu_S = nu_S.detach().cpu().numpy()

        fname = outpath+imgfile.split('/')[-1].replace('.img','.npz').replace('.nii.gz','.npz').replace('.tif','.npz').replace('.TIF','.npz')
