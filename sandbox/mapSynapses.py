...
def main():
    ...
    imageValsS = [
        np.sum(nu_S, axis=-1),
        (np.argmax(nu_S, axis=-1) + 1) * (np.sum(nu_S, axis=-1) > 0),
    ]
    imageValsT = [
        np.sum(nu_T, axis=-1),
        (np.argmax(nu_T, axis=-1) + 1) * (np.sum(nu_T, axis=-1) > 0),
    ]
    ...
    for t in range(len(Dlist)):
        ...
        imageValsD = [
            np.sum(nu_D, axis=-1),
            (np.argmax(nu_D, axis=-1) + 1) * (np.sum(nu_D, axis=-1) > 0),
        ]
