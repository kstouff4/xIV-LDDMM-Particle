...
def main():
    ...
    nu_S[
        np.arange(N), np.random.randint(low=0, high=labs, size=N)
    ] = 1.0  # discrete particles
    nu_S = torch.tensor(nu_S).type(dtype)
    T = (torch.clone(S) * alpha).type(dtype)
    nu_T = (torch.clone(nu_S) * (alpha) ** d).type(dtype)

    savedir = (
        "/cis/home/kstouff4/Documents/MeshRegistration/ParticleLDDMMQP/sandbox/output_dl_sig_its_albe_N-"
        + str(d)
        + str(labs)
        + "_"
        + str(sigmaRKHS)
    ...
    Dlist, nu_Dlist = callOptimize(
        S,
        nu_S,
        T,
        nu_T,
        torch.tensor(sigmaRKHS).type(dtype),
        torch.tensor(sigmaVar).type(dtype),
        d,
        labs,
        savedir,
        its=its,
    ...
    vtf.writeVTK(
        pointList, [], [], savedir + "testOutput_curves.vtk", polyData=polyList
    )
