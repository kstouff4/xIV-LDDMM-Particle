...
def main():
    ...
    S,nu_S = gI.makeFromSingleChannelImage(b2,0.125*4,bg=0,ordering=[8,7],ds=4,weights=torch.tensor(0.5**3).type(dtype))
    Sa,nu_Sa = gI.makeFromSingleChannelImage(b2a,0.125*4,bg=0,ordering=[1,2,3,4],ds=4,weights=torch.tensor(0.5**3).type(dtype))
    ...
    for t in range(len(Dlist)):
        ...
        if (t == len(Dlist) - 1):
            np.savez(savedir+'testOutput.npz',S=S, nu_S=nu_S,T=T,nu_T=nu_T,D=D,nu_D=nu_D,nu_Scomb=nu_Scomb)
