...
def main():
    ...
    sigmaRKHS = [
        0.2,
        0.1,
    ]  # as of 3/16, should be fraction of total domain of S+T #[10.0]
    sigmaVar = [
        0.5,
        0.2,
        0.05,
    ...
    imgFile = (
        imgPref + "210929/AMYGDALA+ERC+TEC.img"
    )  # WHITAM is 150929, 170921, 191001, 210929
    im = nib.load(imgFile)
    ...
    X, Y, Z = torch.meshgrid(
        torch.tensor(x0).type(dtype),
        torch.tensor(x1).type(dtype),
        torch.tensor(x2).type(dtype),
        indexing="ij",
    )
    ...
    # single ERC label in source
    nu_Sn = torch.zeros((nu_S.shape[0], 2)).type(dtype)
    nu_Sn[:, 1] = torch.sum(nu_S[:, 1:], dim=-1)
    nu_Sn[:, 0] = nu_S[:, 0]
    nu_S = nu_Sn
    labS = nu_S.shape[-1]
    nu_T = nu_S
    nu_T = nu_T * torch.tensor(1.0**3).type(
        dtype
    )  # assume 1 mm resolution for biocard and adni
    T = S
    labs = nu_T.shape[-1]
    ...
    S, nu_S = gI.makeFromSingleChannelImage(
        ...
        ordering=[8, 7],
        ds=4,
        weights=torch.tensor(0.5**3).type(dtype),
    )
    Sa, nu_Sa = gI.makeFromSingleChannelImage(
        b2a,
        0.125 * 4,
        bg=0,
        ordering=[1, 2, 3, 4],
    ...
    S = Scomb
    nu_S = nu_Scomb
    nu_S = torch.zeros((nu_Scomb.shape[0], 2)).type(dtype)
    nu_S[:, 0] = nu_Scomb[:, 0] + nu_Scomb[:, 1] + nu_Scomb[:, 2] + nu_Scomb[:, 3]
    nu_S[:, 1] = nu_Scomb[:, 5] + nu_Scomb[:, 4]

    # flip brain 2 template to other side
    Arot = torch.zeros((3, 3)).type(dtype)
    Arot[0, 0] = 1.0
    Arot[1, 1] = -1.0
    Arot[2, 2] = 1.0
    S, nu_S = init.applyAffine(S, nu_S, Arot, torch.zeros((1, d)).type(dtype))
    ...
    nu_Scomb = nu_Scomb.detach().cpu().numpy()
    ...
    vtf.writeVTK(
        S, imageValsS, imageNamesS, savedir + "testOutput_S.vtk", polyData=None
    )
    vtf.writeVTK(
        T, imageValsT, imageNamesT, savedir + "testOutput_T.vtk", polyData=None
    )
    np.savez(
        savedir + "origST.npz",
        S=S,
