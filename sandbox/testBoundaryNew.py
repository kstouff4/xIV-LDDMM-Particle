...
def main():
    ...
    nu_S = torch.stack((nu_S[..., 0].flatten(), nu_S[..., 1].flatten()), axis=-1).type(
        dtype
    )
    nu_T = torch.stack((nu_T[..., 0].flatten(), nu_T[..., 1].flatten()), axis=-1).type(
        dtype
    )

    savedir = (
        "/cis/home/kstouff4/Documents/MeshRegistration/ParticleLDDMMQP/sandbox/CubeTest/output_dl_sig_its_albe_N-"
        + str(d)
        + str(labs)
        + "_"
        + str(sigmaRKHS)
    ...
    Arot = init.get3DRotMatrix(
        torch.tensor(np.pi / 4.0),
        torch.tensor(np.pi / 2.0),
        torch.tensor(3.0 * np.pi / 2.0),
    )
    ...
    getLocalDensity(S, nu_S, sigmaVar[0], savedir + "density_S.vtk")
    getLocalDensity(T, nu_T, sigmaVar[0], savedir + "density_T.vtk")
    getLocalDensity(Dlist[-1], nu_Dlist[-1], sigmaVar[0], savedir + "density_D.vtk")
