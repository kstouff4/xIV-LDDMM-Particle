...
def LDDMMloss(
    K0, sigma, d, numS, gamma, dataloss, cA=1.0, cT=1.0, dimEff=3, single=False
):
    def loss(p0, q0):
        p, q = Shooting(
...
def callOptimize(
    ...
    if pTil is None:
        ...
        pTilde[0:numS] = torch.squeeze(torch.tensor(1.0 / (dimEff * w_S))).type(
            dtype
        )  # torch.sqrt(kScale)*torch.sqrt(kScale)
        pTilde[numS : numS * (d + 1)] = torch.tensor(1.0).type(
            dtype
    ...
    loss = LDDMMloss(
        Kg, sigmaRKHS, d, numS, gamma, dataloss, cA, cT, dimEff, single=single
    )
