...
def Hamiltonian(K0, sigma, d, numS, gammaA, gammaT, gammaU):
    ...
    def H(p, q):
        ...
        A, tau = getATau(
            px, qx, qw, gammaA, gammaT
        )  # getAtau( = (1.0/(2*alpha))*(px.T@(qx-qc) - (qx-qc).T@px) # should be d x d
        Anorm = (A * A).sum()
        print(
            "Anorm is, ",
            (torch.clone(gammaA).cpu().numpy() / 2.0)
            * torch.clone(Anorm).detach().cpu().numpy(),
        )
        print("tau is, ", torch.clone(tau).detach().cpu().numpy())
        print(
            "tauNorm is, ",
            (torch.clone(gammaT).cpu().numpy() / 2.0)
            * (
                np.sum(
                    torch.clone(tau).detach().cpu().numpy()
                    * torch.clone(tau).detach().cpu().numpy()
                )
            ),
        )
        ...
        return (
            (gammaU) * k.sum()
            + (gammaA / 2.0) * Anorm
            + (gammaT / 2.0) * (tau * tau).sum()
        )  # h.sum() + 0.5*torch.sum(pc*pc) + torch.sum(h2)
...
def getATau(px, qx, qw, alpha, gamma):
    ...
    A = ((1.0 / (2.0 * alpha)) * (px.T @ (qx - xc) - (qx - xc).T @ px)).type(
        dtype
    )  # should be d x d
...
def HamiltonianSystemGrid(K0, sigma, d, numS, gammaA, gammaT, gammaU, uCoeff):
    ...
    def HS(p, q, qgrid, qgridw):
        ...
        """
        Gg = (
            (1.0 / gammaU) * GaussKernelU(sigma, d, uCoeff)(gx, qx, px, pw * qw)
            + (gx - xc) @ A.T
            + tau
        ).flatten()
        Ggw = (
            (1.0 / gammaU) * GaussKernelUdiv(sigma, d, uCoeff)(gx, qx, px, pw * qw) * gw
        ).flatten()
...
def Shooting(
    ...
    return Integrator(
        HamiltonianSystem(K0, sigma, d, numS, gammaA, gammaT, gammaU), (p0, q0), nt
    )
...
def LDDMMloss(K0, K1, sigma, d, numS, gammaA, gammaT, gammaU, dataloss, c=1.0):
    def loss(p0, q0):
        ...
        return c * Hamiltonian(K0, sigma, d, numS, gammaA, gammaT, gammaU)(
            p0, q0
        ), dataloss(q)
        # return dataloss(q)
...
def ShootingGrid(
    ...
    return Integrator(
        HamiltonianSystemGrid(K0, sigma, d, numS, gammaA, gammaT, gammaU, uCoeff),
        (p0, q0, qGrid, qGridw),
        nt,
    )
...
def lossVarifoldNorm(T, w_T, zeta_T, zeta_S, K, d, numS, beta):
    ...
    def loss(sS):
        ...
        return (1.0 / 2.0) * (cst + k1.sum() - 2 * k2.sum())
...
def callOptimize(
    ...
    cScale=torch.tensor(10.0).type(dtype),
    beta=None,
):
    """
    Parameters:
    ...
    """
    w_S, w_T, zeta_S, zeta_T, q0, p0, numS, Stilde, Ttilde, s, m = makePQ(
        S, nu_S, T, nu_T
    )
    ...
    if beta is None:
        ...
        if len(sigmaVar) == 1:
            Kinit = GaussLinKernel(
                sigma=sigmaVar, d=d, l=labs, beta=[torch.tensor(1.0).type(dtype)]
            )
        ...
        # print out indiviual costs
            ...
            for sig in sigmaVar:
                ...
                Kinit = GaussLinKernel(
                    sigma=[sig], d=d, l=labs, beta=[torch.tensor(1.0).type(dtype)]
                )
                cinit = Kinit(Ttilde, Ttilde, w_T * zeta_T, w_T * zeta_T).sum()
    ...
    for sig in sigmaRKHS:
        ...
        uCoeff.append(
            torch.clone((1.0 / (N * N * sig * sig)) * Kinit(Stilde, Stilde).sum()).type(
                dtype
            )
        )
    ...
    ax.plot(
        ...
        label="H($q_0$,$p_0$), Final = {0:.6f}".format(lossListH[-1]),
    )
    ax.plot(
        np.arange(len(lossListH)),
        np.asarray(lossListDA),
        label="Varifold Norm, Final = {0:.6f}".format(lossListDA[-1]),
    )
    ax.plot(
        np.arange(len(lossListH)),
        np.asarray(lossListDA) + np.asarray(lossListH),
        label="Total Cost, Final = {0:.6f}".format(lossListDA[-1] + lossListH[-1]),
    )
    ax.set_title("Loss")
    ...
    f, ax = plt.subplots()
    ax.plot(
        np.arange(len(lossOnlyH)),
        np.asarray(lossOnlyH),
        label="TotLoss, Final = {0:.6f}".format(lossOnlyH[-1]),
    )
    ...
    f, ax = plt.subplots()
    ax.plot(
        np.arange(len(lossListHdiff)),
        np.asarray(lossListHdiff),
        label="H($q_0$,$p_0$), Final = {0:.6f}".format(lossListHdiff[-1]),
    )
    ax.plot(
        np.arange(len(lossListHdiff)),
        np.asarray(lossListDAdiff),
        label="Varifold Norm, Final = {0:.6f}".format(lossListDAdiff[-1]),
    )
    ax.plot(
        np.arange(len(lossListHdiff)),
        np.asarray(lossListTotdiff),
        label="Total Cost, Final = {0:.6f}".format(lossListTotdiff[-1]),
    )
    ...
    np.savez(
        savedir + "testOutput_values.npz",
        A0=A.detach().cpu().numpy(),
        tau0=tau.detach().cpu().numpy(),
