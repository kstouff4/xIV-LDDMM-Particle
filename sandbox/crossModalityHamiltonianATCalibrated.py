...
def HamiltonianSystemGrid(K0, sigma, d, numS, uCoeff, cA=1.0, cT=1.0):
    ...
    def HS(p, q, qgrid, qgridw):
        ...
        Gg = (
            getU(sigma, d, uCoeff)(gx, qx, px, pw * qw) + (gx - xc) @ A.T + tau
        ).flatten()
...
def Shooting(
    p0, q0, K0, sigma, d, numS, cA=1.0, cT=1.0, nt=10, Integrator=RalstonIntegrator()
):
...
def LDDMMloss(K0, sigma, d, numS, gamma, dataloss, piLoss, cA=1.0, cT=1.0, cPi=10.0):
    def loss(p0, q0):
        ...
        return (
            (gamma * Hamiltonian(K0, sigma, d, numS, cA, cT)(p0[: (d + 1) * numS], q0)),
            dataloss(q, p0[(d + 1) * numS :]),
...
def ShootingGrid(
    ...
    return Integrator(
        HamiltonianSystemGrid(K0, sigma, d, numS, uCoeff, cA, cT),
...
def printCurrentVariables(
    p0Curr, itCurr, K0, sigmaRKHS, uCoeff, q0Curr, d, numS, zeta_S, labT, s, m, savedir
):
    np.savez(
        savedir + "p0_iter" + str(itCurr) + ".npz", p0=p0Curr.detach().cpu().numpy()
    )
    ...
    vtf.writeVTK(
        ...
        imageValsS,
        imageNamesS,
        savedir + "testOutputiter" + str(itCurr) + "_D10.vtk",
        polyData=None,
    )
    vtf.writeVTK(
        D,
        imageValsSpi,
        imageNamesSpi,
        savedir + "testOutputiter" + str(itCurr) + "_Dpi10.vtk",
        polyData=None,
    )
    return
...
def makePQ(S, nu_S, T, nu_T, norm=True):
    ...
    p0 = (
        torch.cat(
            (torch.zeros_like(q0), torch.sqrt(pi_STinit).clone().detach().flatten()), 0
        )
        .requires_grad_(True)
        .type(dtype)
    )
...
def callOptimize(
    ...
    """
    w_S, w_T, zeta_S, zeta_T, q0, p0, numS, Stilde, Ttilde, s, m, pi_STinit = makePQ(
        S, nu_S, T, nu_T
    )
    ...
    pTilde[numS:] = torch.tensor(1.0).type(
        dtype
    ...
    if beta is None:
        ...
        if len(sigmaVar) == 1:
            ...
            k1 = Kinit(
                Stilde, Stilde, (w_S * zeta_S) @ pi_STinit, (w_S * zeta_S) @ pi_STinit
            )
        ...
        # print out indiviual costs
            ...
            for sig in sigmaVar:
                ...
                k1 = Kinit(
                    Stilde,
                    Stilde,
                    (w_S * zeta_S) @ pi_STinit,
                    (w_S * zeta_S) @ pi_STinit,
                ).sum()
                k2 = (
                    -2.0
                    * Kinit(
                        Stilde, Ttilde, (w_S * zeta_S) @ pi_STinit, w_T * zeta_T
                    ).sum()
                )
                beta.append(
    ...
    for i in range(its):
        ...
        if np.mod(i, 25) == 0:
            ...
            printCurrentVariables(
                p0, i, Kg, sigmaRKHS, uCoeff, q0, d, numS, zeta_S, labs, s, m, savedir
            )
    ...
    ax.plot(
        np.arange(len(lossListH)),
        np.asarray(lossListPI),
        label="Pi Cost, Final = {0:.6f}".format(lossListPI[-1]),
    )
    ax.set_title("Loss")
    ...
    listpq = ShootingGrid(
        p0 * pTilde, q0, qGrid, qGridw, Kg, sigmaRKHS, d, numS, uCoeff, cA, cT
    )
    ...
    listSp0[numS:, :] = (
        p0[numS : (d + 1) * numS].detach().view(-1, d).cpu().numpy()
        + listSp0[0:numS, :]
    )
