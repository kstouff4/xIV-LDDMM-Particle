...
def printTransformations(p0, savedir, it):
    ...
    f, ax = plt.subplots()
    ax.plot(
        np.arange(len(thetaTot)),
        thetaTot,
        label="angle (deg): mean = {0:.6f}, std = {1:.6f}".format(
            np.mean(thetaTot), np.std(thetaTot)
        ),
    )
    ax.plot(
        np.arange(len(tauTotX)),
        tauTotX,
        label="X translation (mm): mean = {0:.6f}, std = {1:.6f}".format(
            np.mean(tauTotX), np.std(tauTotX)
        ),
    )
    ax.plot(
        np.arange(len(tauTotY)),
        tauTotY,
        label="Y translation (mm): mean = {0:.6f}, std = {1:.6f}".format(
            np.mean(tauTotY), np.std(tauTotY)
        ),
    )
    ax.legend()
    ...
    ax.plot(
        ...
        np.abs(thetaTot),
        label="angle (deg): mean = {0:.6f}, std = {1:.6f}".format(
            np.mean(np.abs(thetaTot)), np.std(np.abs(thetaTot))
        ),
    )
    ax.plot(
        np.arange(len(tauTotX)),
        np.abs(tauTotX),
        label="X translation (mm): mean = {0:.6f}, std = {1:.6f}".format(
            np.mean(np.abs(tauTotX)), np.std(np.abs(tauTotX))
        ),
    )
    ax.plot(
        np.arange(len(tauTotY)),
        np.abs(tauTotY),
        label="Y translation (mm): mean = {0:.6f}, std = {1:.6f}".format(
            np.mean(np.abs(tauTotY)), np.std(np.abs(tauTotY))
        ),
    )
    ax.legend()
...
def align(Slist, nu_Slist, sigma, its, savedir, norm=False):
        ...
        def loss(p0):
            for i in range(len(Slist) - 2):
                ...
                if i == 0:
                    L = (1.0 / c) * K(
                        Slist[i],
                        applyRigid2D(Slist[i + 1], nu_Slist[i + 1], theta, tau),
                        nu_Slist[i],
                        nu_Slist[i + 1],
                    ).sum()
                else:
            ...
            L += (1.0 / c) * K(
                applyRigid2D(
                    Slist[-2], nu_Slist[-2], p0[-3].view(1, 1), p0[-2:].view(1, 2)
                ),
                Slist[-1],
                nu_Slist[-2],
                nu_Slist[-1],
    ...
    f, ax = plt.subplots()
    ax.plot(
        np.arange(len(lossList)),
        np.asarray(lossList),
        label="Total Cost, Final = {0:.6f}".format(lossList[-1]),
    )
    ax.legend()
    ...
    ax.plot(
        np.arange(len(lossList)),
        -1.0 * np.log(np.asarray(lossList) * -1.0),
        label="Total Cost, Final = {0:.6f}".format(lossList[-1]),
    )
    ax.legend()
    ...
    for i in range(len(Slist) - 2):
        Snew.append(
            applyRigid2D(
                Slist[i + 1], nu_Slist[i + 1], p0[i * 3], p0[1 + i * 3 : (i + 1) * 3]
            )
        )
