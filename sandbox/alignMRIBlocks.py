...
def getFinalTransforms(p0, Slist, nuSlist, lamb, numVars=6, numTrans=3):
    ...
    for i in range(numTrans):
        ...
        angles.append(p[0:3])
        ts.append(p[3:] * lamb)
        AS, nuAS = init.applyAffine(
            Slist[i + 1], nuSlist[i + 1], A, p[3:] * lamb, bc=False
        )
        ASlist.append(AS)
...
def callOptimize(Slist, nu_Slist, sigmaVar, gammaR, savedir, its=100, d=3, numVars=6):
    ...
    """
    p0, Spost, nuSpost, Sant, nuSant, lamb = makePQ(
        Slist, nu_Slist, numVars, len(Slist) - 1
    )
    ...
    if len(sigmaVar) == 1:
        ...
        cinit = Kinit(
            torch.cat(Spost), torch.cat(Sant), torch.cat(nuSpost), torch.cat(nuSant)
        ).sum()
    ...
    dataloss = lossVarifoldNorm(
        GaussLinKernel(sigma=sigmaVar, d=d, l=labs, beta=beta), nuSant, nuSpost
    )
