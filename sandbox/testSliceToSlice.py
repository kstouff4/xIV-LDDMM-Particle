...
def main():
    ...
    filesO = [
        "/cis/home/kstouff4/Documents/MeshRegistration/Particles/BarSeq/Redo__optimalZnu_ZAllwC8.0_sig[0.2]_Nmax1500.0_Npart2000.0.npz"
    ]

    filesO = [
        "/cis/home/kstouff4/Documents/MeshRegistration/Particles/BarSeq/top28MI/sig0.025/allSlices_lowToHighMI_centeredAndScaled0.001_US_optimal_all.npz"
    ]
    extra = "BarSeq0.025/"
    """
    ...
    Snew, thetas, taus = sts.align(
        Slist, nu_Slist, torch.tensor(sigma).type(dtype), its, savedir, norm=False
    )
    ...
    for s in range(len(Snew)):
        ...
        np.savez(
            savedir + "slice_" + str(s) + ".npz",
            S=SnewTotal[count : count + S.shape[0]],
            nu_S=nu_S,
        )
        gO.writeVTK(
            SnewTotal[count : count + S.shape[0]],
            [np.sum(nu_S, axis=-1), np.argmax(nu_S, axis=-1)],
            ["totalMass", "maxVal"],
            savedir + "slice_" + str(s) + ".vtk",
            polyData=None,
        )
    ...
    gO.writeVTK(
        SnewTotal, imageVals, imageNames, savedir + "allSlices.vtk", polyData=None
    )
