...
def main():
    ...
    outpath = (
        "/cis/home/kstouff4/Documents/MeshRegistration/ParticleLDDMMQP/sandbox/AllenMERFISH/"
        + alphaSt
        + "/"
    )
    ...
    Dlist, nu_Dlist, nu_DPilist, Glist, nu_Glist, Tlist, nu_Tlist = callOptimize(
        ...
        cPi=cPi,
        dimEff=dimEff,
        Csqpi=torch.tensor(Csqpi).type(dtype),
    )
    ...
    vtf.writeVTK(
        Td, imageValsT, imageNamesT, savedir + "testOutput_Td.vtk", polyData=None
    )
    ...
    gO.getJacobian(
        Dlist[-1], nu_S, nu_Dlist[-1], savedir + "testOutput_D10_jacobian.vtk"
    )
