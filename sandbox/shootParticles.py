...
def makeGrid(T, nu_T, res, savedir, dimE=2):
    ...
    mIm = nib.Nifti1Image(
        (np.argmax(nu_Xgrid, axis=-1) + 1) * (np.sum(nu_Xgrid, axis=-1) > 0),
        np.eye(4),
        empty_header,
    )
...
def shootForwards(paramFile, variableFile, Q, w_Q, dimEff=3):
    ...
    listpq = ShootingGrid(
        p0, q0, Q, w_Q, Kg, sigmaRKHS, d, numS, uCoeff, cA, cT, dimEff, single=single
    )
...
def shootTargetGridSlice(Ttildei, nu_T, index, res, savedir, paramFile, variableFile):
    ...
    if index >= 0:
        ...
        X, nu_X, Xgrid, nu_Xgrid = makeGrid(
            T.cpu().numpy(),
            nuT.cpu().numpy(),
            res,
            savedir + "Slice_" + str(index) + "_",
            dimE=2,
        )
        ...
        Xs, w_Xs = shootBackwards(
            paramFile,
            variableFile,
            torch.tensor(X).type(dtype),
            torch.tensor(w_X).type(dtype),
            dimE=3,
        )
        ...
        np.savez(
            savedir + "Slice_" + str(index) + "_originalAndShot.npz",
            Xs=Xs,
            w_Xs=w_Xs,
            X=X,
            nu_X=nu_X,
...
def constructVelocityFields(paramFile, variableFile, outpath, dimEff=3):
    ...
    gO.writeVectorField(
        XT, vT, outpath + "target_to_source_velocityField" + str(res) + ".vtk"
    )
    np.savez(
        outpath + "target_to_source_velocityField" + str(res) + ".npz",
        startPos=XT,
        endPos=vT,
        startW=qGridw.detach().cpu().numpy(),
        endW=wT,
    )
    ...
    XT = init.resizeData(XT.detach().view(-1, d).cpu().numpy(), s, m)
    gO.writeVectorField(
        XT, vT, outpath + "source_to_target_velocityField" + str(res) + ".vtk"
    )
    np.savez(
        outpath + "source_to_target_velocityField" + str(res) + ".npz",
        startPos=XT,
        endPos=vT,
        startW=qGridw.detach().cpu().numpy(),
        endW=wT,
