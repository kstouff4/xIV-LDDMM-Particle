...
def makeImages(S, nu_S, Td, nu_Td, sigma, featNamesS, featNamesT, savedir):
    ...
    nu_Gsim = np.reshape(
        nu_Gs, (X0.shape[0], X0.shape[1], X0.shape[2], nu_Gs.shape[-1])
    )

    empty_header = nib.Nifti1Header()
    nu_GsMV = nib.Nifti1Image(
        (np.argmax(nu_Gsim, axis=-1) + 1) * (np.sum(nu_Gsim, axis=-1) > 0.001),
        np.eye(4),
        empty_header,
    )
    ...
    nu_Gtim = np.reshape(
        nu_Gt, (X0.shape[0], X0.shape[1], X0.shape[2], nu_Gt.shape[-1])
    )

    nu_GtSum = nib.Nifti1Image(np.sum(nu_Gtim, axis=-1), np.eye(4), empty_header)
    nu_GtMV = nib.Nifti1Image(
        (np.argmax(nu_Gtim, axis=-1) + 1) * (np.sum(nu_Gtim, axis=-1) > 0.001),
        np.eye(4),
        empty_header,
    )
    ...
    if featNamesT is not None:
        for f in range(nu_Gt.shape[-1]):
            nu_Gtf = nib.Nifti1Image(
                np.squeeze(nu_Gtim[..., f] * denom), np.eye(4), empty_header
            )
            nib.save(
                nu_Gtf, savedir + "nu_Tf" + str(f) + "_" + featNamesT[f] + ".nii.gz"
            )
    ...
    if featNamesS is not None:
        for f in range(nu_Gs.shape[-1]):
            nu_Gtf = nib.Nifti1Image(
                np.squeeze(nu_Gsim[..., f] * denom), np.eye(4), empty_header
            )
            nib.save(
                nu_Gtf, savedir + "nu_Sf" + str(f) + "_" + featNamesS[f] + ".nii.gz"
            )
...
def main():
    ...
    savedir = (
        "/cis/home/kstouff4/Documents/MeshRegistration/ParticleLDDMMQP/sandbox/BarSeq/AllenAtlas200ToBarSeq/output_dl_sig_its_albega_N-35_[0.2, 0.1, 0.05][0.5, 0.2, 0.05, 0.02]_100_0.1None_67827flipFullAtlas/sig"
        + str(sigma)
        + "gridIm_"
    )
    ...
    Xgs, X0s = makeImages(
        S, nu_S, Td, nu_Td, [sigma, sigma, sigma], None, featNamesT, savedir + "CCF_"
    )
    interpolateOnAtlasImage(
        Xgs, np.stack((X0s, X0s, X0s), axis=-1), savedir + "CCFimage_"
    )
    Xg, X0 = makeImages(
        T, nu_T, D, nu_D, [sigma, sigma, sigma], featNamesT, None, savedir + "BarSeq_"
    )
