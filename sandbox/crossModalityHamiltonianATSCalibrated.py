...
def callOptimize(
    ...
    for i in range(its):
        ...
        if np.mod(i, 20) == 0:
            ...
            printCost(i)
            checkEndPoint(
                dataloss, p0 * pTilde, p1, q1, d, numS, savedir + "it" + str(i)
            )
            if i > 0:
                if np.allclose(
                    lossListH[-1], lossListH[-2], atol=1e-6, rtol=1e-5
                ) and np.allclose(lossListDA[-1], lossListDA[-2], atol=1e-6, rtol=1e-5):
                    print("state of optimizer")
                    print(osd)
                    break
    ...
    if pi_STfinal.shape[0] > pi_STfinal.shape[1] * 10:
        ...
        for j in range(pi_STfinal.shape[1]):
            pi_STplot[:, j * rat : (j + 1) * rat] = np.squeeze(pi_STfinal[:, j])[
                ..., None
            ]
    else:
    ...
    for t in range(len(listpq)):
        ...
        wTt = listpq[t][5]
        nuTlist.append(
            wTt.detach().cpu().numpy()[..., None] * zeta_T.detach().cpu().numpy()
        )
    ...
    listSp0[numS:, :] = (
        p0T[numS : (d + 1) * numS].detach().view(-1, d).cpu().numpy()
        + listSp0[0:numS, :]
    )
    ...
    np.savez(
        savedir + "testOutput_targetScaled.npz",
        T=Ttilde.detach().cpu().numpy(),
        w_T=w_T.detach().cpu().numpy(),
        zeta_T=zeta_T.detach().cpu().numpy(),
        s=s,
        m=m,
    )
    np.savez(
        savedir + "testOutput_Dvars.npz",
        D=Dlist[-1],
        nu_D=nu_Dlist[-1],
        nu_Dpi=nu_DPilist[-1],
        Td=Tlist[-1],
