...
def LDDMMloss(K0,sigma, d, numS,gamma,dataloss,piLoss,cA=1.0,cT=1.0,cPi=10.0,dimEff=3,single=False):
    def loss(p0, q0):
...
def callOptimize(S,nu_S,T,nu_T,sigmaRKHS,sigmaVar,gamma,d,labs, savedir, its=100,kScale = torch.tensor(1.0).type(dtype),cA=1.0,cT=1.0,cS=10.0,cPi=1.0,dimEff=3,Csqpi=torch.tensor(1.0).type(dtype),single=False,beta=None,loadPrevious=None):
    '''
    Parameters:
