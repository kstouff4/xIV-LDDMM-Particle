...
def GaussKernelGrad(sigma, d):
    ...
    x, y, b = (
        ...
        Vj(1, d) / sigma,
        Vi(2, 1),
    )  # \tilde{x} = x / sigma one dimensional momentum p_w
    gamma = 0.5
    D2 = x.sqdist(y)
    K = (-D2 * gamma).exp()
    Kxy_b = K * b
...
def GaussKernelLaplace(sigma, d):
    ...
    K = (-D2 * gamma).exp()
    b_Kxy_b = bx * K * by
    g = torch.trace(b_Kxy_b.grad(x, 1.0).grad(x, 1.0))
...
def callOptimize(S, nu_S, T, nu_T, sigmaRKHS, sigmaVar, d, labs):
    ...
    dataloss = lossVarifoldNorm(
        T, w_T, zeta_T, zeta_S, GaussLinKernel(sigma=sigmaVar, d=d, l=labs), beta=0.1
    )
